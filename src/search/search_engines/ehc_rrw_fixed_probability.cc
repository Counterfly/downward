#include "ehc_rrw_fixed_probability.h"
...
namespace enforced_hill_climbing_restarting_random_walk_fixed_probability {
...
EnforcedHillClimbingSearchRRWFixedProbability::EnforcedHillClimbingSearchRRWFixedProbability(
    const Options &opts)
    : SearchEngine(opts),
...
void EnforcedHillClimbingSearchRRWFixedProbability::reach_state(
...
// void EnforcedHillClimbingSearchRRWFixedProbability::save_plan_if_necessary() {
//     if (found_solution()) {
//         plan_manager.save_plan(plan, task_proxy);
//     }
// }

void EnforcedHillClimbingSearchRRWFixedProbability::initialize() {
    assert(evaluator);
...
void EnforcedHillClimbingSearchRRWFixedProbability::insert_successor_into_open_list(
...
void EnforcedHillClimbingSearchRRWFixedProbability::expand(EvaluationContext &eval_context) {
...
void EnforcedHillClimbingSearchRRWFixedProbability::get_biased_successors(EvaluationContext &eval_context, ordered_set::OrderedSet<OperatorID> &ops) {
...
bool EnforcedHillClimbingSearchRRWFixedProbability::check_goal_and_set_plan(const GlobalState &state) {
...
void EnforcedHillClimbingSearchRRWFixedProbability::search() {
    initialize();
    status = IN_PROGRESS;
...
SearchStatus EnforcedHillClimbingSearchRRWFixedProbability::step() {
...
SearchStatus EnforcedHillClimbingSearchRRWFixedProbability::ehc() {
    while (!open_list->empty()) {
        ...
        if (node.is_new()) {
            ...
            int h = eval_context.get_evaluator_value(evaluator.get()); // hvalue of successor
            node.open(parent_node, last_op, get_adjusted_cost(last_op));

            if (h < current_eval_context.get_evaluator_value(evaluator.get())) {	// if hvalue of successor < hvalue of parent
            	++num_ehc_phases;
    ...
    do {
        ...
        double random_value = (double)(*rng)();
...
void EnforcedHillClimbingSearchRRWFixedProbability::print_statistics() const {
...
static shared_ptr<SearchEngine> _parse(OptionParser &parser) {
    ...
    parser.add_option<double>("prob", "restart probability (0 <= r <= 1)", "0");
    ...
    if (parser.dry_run())
        ...
        return make_shared<EnforcedHillClimbingSearchRRWFixedProbability>(opts);
}

static Plugin<SearchEngine> _plugin("ehc_rrw_fixed_prob", _parse);
}